...
(let ((db (sqlite-open "metadata.db"))
  ...
  (with-temp-buffer
    ...
    (insert "    VERBOSE=y\n")
    (insert "fi\n")
    (insert "function logp() {\n")
    (insert "    if [ \"$VERBOSE\" == \"y\" -a \"$1\" != \"\" ]; then\n")
    (insert "	printf \"%s...\" \"$1\"\n")
    (insert "    fi\n")
    (insert "}\n")
    (insert "function log() {\n")
    (insert "    if [ \"$VERBOSE\" == \"y\" ]; then\n")
    (insert "	if [ \"$1\" == \"\" ]; then\n")
    (insert "	    echo done\n")
    (insert "	else\n")
    (insert "	    echo \"$1\"\n")
	...
	(let ((dir (book-kebab-case series))
	  ...
	  (unless (member dir dirs)
	    ...
	    (unless first-series
	      (insert (format "log\n")))
	    (setq first-series nil)
	    (insert (format "logp \"%s\"\n" dir))
	    (insert (format "mkdir -p %s\n" target-dir)))
	...
	(insert (format "logp \"%s\"\n" (book-kebab-case (car author-books))))
